...
mock.module("@pocketjs/framework", () => ({
  getOps: () => ({ measureText: (value: string) => [...value].length * 8 }),
}));

const { wrapTextPage } = await import("./text.ts");

test("wraps words and unbroken tokens within maxWidth", () => {
  const page = wrapTextPage("alpha beta 123456789", 2, 48, 0, 0, 10);
...
test("paginates deterministically to EOF and always advances", () => {
  ...
  for (let iteration = 0; iteration <= source.length; iteration += 1) {
    const page = wrapTextPage(source, 2, 40, offset, sourceLine, 2);
    expect(wrapTextPage(source, 2, 40, offset, sourceLine, 2)).toEqual(page);
...
test("tracks source lines across LF and CRLF", () => {
  const source = "aa\nbb\r\ncc";
  const first = wrapTextPage(source, 2, 80, 0, 0, 2);
  const second = wrapTextPage(source, 2, 80, first.nextOffset, first.nextSourceLine, 2);
