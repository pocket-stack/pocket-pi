...
function join(...args) {
  ...
  for (const arg of args) {
    assertPath(arg);
    if (arg.length > 0) joined = joined === void 0 ? arg : joined + "/" + arg;
  }
...
const posix = { sep, delimiter: ":", normalize, isAbsolute, join, resolve, dirname, basename, extname, relative, parse, format, toNamespacedPath };
const win32 = { ...posix, sep: "\\", delimiter: ";" };
const delimiter = ":";
var path_default = { sep, delimiter, normalize, isAbsolute, join, resolve, dirname, basename, extname, relative, parse, format, toNamespacedPath, posix, win32 };
export {
  basename,
