(function() {
        ...
        segment(str) {
          ...
          return {
            [Symbol.iterator]() {
              ...
              return {
                next() {
                  return i < chars.length ? { value: { segment: chars[i], index: i++, input: str }, done: false } : { value: void 0, done: true };
                }
              };
            }
          };
  ...
  if (typeof globalThis.Blob === "undefined") {
    globalThis.Blob = class Blob {
      ...
      slice() {
        return new globalThis.Blob(this._parts, { type: this.type });
      }
    };
  }
  ...
  if (typeof globalThis.FormData === "undefined") {
    globalThis.FormData = class FormData {
      ...
      get(k) {
        const e = this._entries.find((e2) => e2[0] === String(k));
        return e ? e[1] : null;
      }
      getAll(k) {
      ...
      *keys() {
        for (const e of this._entries) yield e[0];
      }
      *values() {
        for (const e of this._entries) yield e[1];
      }
      [Symbol.iterator]() {
        return this.entries();
      }
  ...
  if (typeof globalThis.EventTarget !== "function") {
    globalThis.EventTarget = class EventTarget {
      ...
      removeEventListener(t, cb) {
        this.__l[t] = (this.__l[t] || []).filter((f) => f !== cb);
      }
      dispatchEvent(e) {
        for (const cb of this.__l[e && e.type] || []) {
          try {
            cb(e);
          } catch {
          }
  ...
  if (typeof globalThis.Headers !== "function") {
    globalThis.Headers = class Headers {
      ...
      append(k, v) {
        const p = this._m.get(String(k).toLowerCase());
        this.set(k, p ? p + ", " + v : v);
      }
      get(k) {
  ...
  if (typeof globalThis.URLSearchParams !== "function") {
    globalThis.URLSearchParams = class URLSearchParams {
      ...
      get(k) {
        const e = this._p.find((x) => x[0] === k);
        return e ? e[1] : null;
      }
      set(k, v) {
        const e = this._p.find((x) => x[0] === k);
        if (e) e[1] = String(v);
        else this._p.push([k, String(v)]);
      }
      append(k, v) {
        this._p.push([k, String(v)]);
      }
      has(k) {
        return this._p.some((x) => x[0] === k);
      }
      delete(k) {
        this._p = this._p.filter((x) => x[0] !== k);
      }
      forEach(fn) {
  ...
  if (typeof globalThis.URL !== "function") {
    ...
    globalThis.URL = class URL {
      ...
      get href() {
        return this.origin + this.pathname + (this.searchParams.toString() ? "?" + this.searchParams.toString() : "") + this.hash;
      }
      toString() {
        return this.href;
      }
  ...
  class PPReadableStream {
    ...
    _flush() {
      while (this._waiters.length) {
        ...
        else if (this._closed) this._waiters.shift().resolve({ value: void 0, done: true });
        else break;
      }
    }
    getReader() {
    ...
    [Symbol.asyncIterator]() {
      ...
      return { next: () => reader.read(), return: () => {
        reader.cancel();
  ...
  class Response {
    ...
    async arrayBuffer() {
      return (await this._consume()).buffer;
    }
    async text() {
    ...
    clone() {
      return new Response(this.body, { status: this.status, statusText: this.statusText, headers: this.headers, url: this.url });
    }
  }
  globalThis.Response = globalThis.Response || Response;
  const fetchTurns = /* @__PURE__ */ new Map();
