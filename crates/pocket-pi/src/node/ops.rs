...
fn install_node_ns(ctx: &Ctx) -> rquickjs::Result<()> {
    ...
    node.set(
        "hostname",
        Function::new(ctx.clone(), || -> String { "localhost".into() })?,
    )?;

    // Resolution + file read for the synchronous CJS `require`.
    node.set(
        "resolve",
        Function::new(ctx.clone(), |from: String, spec: String| -> String {
...
fn make_fs<'js>(ctx: &Ctx<'js>) -> rquickjs::Result<Object<'js>> {
    ...
    fs.set(
        "writeFile",
        Function::new(ctx.clone(), |path: String, bytes: Vec<u8>| -> String {
    ...
    fs.set(
        "mkdir",
        Function::new(ctx.clone(), |path: String, recursive: bool| {
            let _ = if recursive {
...
fn spawn_sync(cmd: String, args_json: String, opts_json: String) -> String {
    ...
    match command.output() {
        ...
        .to_string(),
        Err(e) => serde_json::json!({ "error": e.to_string(), "status": serde_json::Value::Null })
            .to_string(),
    }
}
