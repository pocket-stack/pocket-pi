// node:https — stub; Pocket Pi routes HTTP through fetch(). Imports resolve; the
...
export function request() { throw new Error("https.request not supported (use fetch)"); }
export function get() { throw new Error("https.get not supported (use fetch)"); }
